...
export default function RegisterButton() {
  const handleClick = async () => {
    try {
      ...
      const accommodationType =
        localStorage.getItem("accommodation_type") || "";
      const spaceType = localStorage.getItem("space_type") || ""; // Corrigido para space_type
      const address = localStorage.getItem("address") || "";
      const referenceAddress = localStorage.getItem("reference_address") || "";
      const guestNumber = parseInt(
        localStorage.getItem("guest_number") || "0",
        10
      ); // Nome de variável ajustado
      const numberRooms = parseInt(
        localStorage.getItem("number_rooms") || "0",
        10
      );
      const numberBeds = parseInt(
        localStorage.getItem("number_beds") || "0",
        10
      );
      const numberBathrooms = parseInt(
        localStorage.getItem("number_bathrooms") || "0",
        10
      );
      const conveniences = JSON.parse(
        localStorage.getItem("conveniences") || "[]"
      ) as string[];
      ...
      const inscricaoEstadual =
        localStorage.getItem("state_registration") || "";
      const dataFundacao = localStorage.getItem("date_of_foundation") || "";
      const dataNascimento = localStorage.getItem("date_of_birth") || "";
      const formData = JSON.parse(localStorage.getItem("formData") || "{}"); // Simplificado para evitar repetição
      ...
      const requestBody = {
        ...
        conveniences,
        photos,
        title: formData.title || "", // Acessa diretamente o título de formData
        description: formData.description || "", // Acessa diretamente a descrição de formData
      };
  ...
  const adicionar = async (body: any) => {
    try {
      ...
      console.log("Dados enviados com sucesso."); // Adiciona uma mensagem de sucesso
    } catch (error) {
      console.error("Erro ao enviar dados para a API:", error);
      throw error; // Reenvia o erro para que ele possa ser tratado adequadamente em níveis superiores, se necessário
    }
  };
